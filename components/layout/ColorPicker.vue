  ...
  <UPopover :close-delay="300" mode="hover">
    ...
    <template #panel>
      <div class="grid grid-cols-5 p-2">
        <UButton v-for="color in primaryColors" :key="color.text" color="white" square :variant="primary?.text === color.text ? 'solid' : 'ghost'" @click="primary = color">
          <span class="inline-block rounded-full w-3 h-3" :style="{ backgroundColor: color.hex }" />
        </UButton>
      </div>
      <hr class="border-gray-200 dark:border-gray-800">
      <div class="grid grid-cols-5 p-2">
        <UButton v-for="color in grayColors" :key="color.text" color="white" square :variant="gray?.text === color.text ? 'solid' : 'ghost'" @click="gray = color">
          <span class="inline-block rounded-full w-3 h-3" :style="{ backgroundColor: color.hex }" />
        </UButton>
      </div>
    </template>
